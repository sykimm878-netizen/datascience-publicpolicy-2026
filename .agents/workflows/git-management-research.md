---
name: git-management-research
description: Commit Management Workflow tailored for Research projects.
tools: Bash, Read, AskUserQuestion
model: sonnet
---
# Commit Management Workflow: Research Projects

## Objective
Generate git commit messages that strictly adhere to the project's historical ALL-CAPS verb convention while ensuring atomicity and proper review gates for experiments, data pipelines, model training, and analytical reproductions.

## Commit Pattern
Commit messages MUST follow this format:
1.  **ALL-CAPS Verb:** Start with a strong action verb from the dictionary.
2.  **Concise Subject:** A short description of the change (e.g., pipeline stage, parameter tuning, model definition).
3.  **No Trailing Punctuation:** Do not end with a period.
4.  **Standard References:** Use lowercase for file extensions (e.g., `.py`, `.yaml`, `.parquet`, `.ipynb`).

### Action Verb Dictionary
`CREATE`, `UPDATE`, `ADD`, `REMOVE`, `FIX`, `REFACTOR`, `DEPRECATE`, `DOCS`, `EXPERIMENT`, `EVAL`.

### Examples
*   `ADD statsmodels VAR analysis to pipeline`
*   `UPDATE params.yaml with new rolling window sizes`
*   `REFACTOR sentiment feature extraction`
*   `EXPERIMENT run local projections on subset data`
*   `FIX dvc stage dependencies for data cleaner`
*   `EVAL compute RMSE for out-of-sample forecasts`
*   `DOCS draft methodology section for survey paper`
*   `CREATE initial EDA jupyter notebook for yield curves`
...
*   **Group Couplings:** Bundle logically related changes (e.g., an updated python script `src/pipeline.py` and its corresponding configuration in `params.yaml` and `dvc.yaml`).
*   **Differentiate Intent:** Keep model changes (`EXPERIMENT`/`REFACTOR`) separate from exploratory notebooks (`ADD` notebooks) and report drafting (`DOCS`) unless intrinsically linked.
...
*   **Testing:** Any commands needed to verify (e.g., dvc repro)
